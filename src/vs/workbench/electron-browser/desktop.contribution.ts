...
(function registerJSONSchemas(): void {
	...
	jsonRegistry.registerSchema(argvDefinitionFileSchemaId, {
		...
		properties: {
			...
			'force-color-profile': {
				type: 'string',
				markdownDescription: nls.localize('argv.forceColorProfile', 'Allows to override the color profile to use. If you experience colors are drawn badly, try to set this to `srgb` and restart.')
			}
		}
	});
})();
